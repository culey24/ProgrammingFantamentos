...
{
    attackScore = myCeil(((static_cast<int>(vehicleType) * 304) + (quantity * weight))/30.0);
    return attackScore;
}
...
bool Army::isSpecialNumber(int n, int k) {
    if (n <= 0) return false;
    while (n > 0) {
        int remainder = n % k;
        if (remainder > 1) {
...
Army::Army(Unit **unitArray, int size, string name, BattleField* battleField) : name(name), battleField(battleField)
{
    ...
    if (isSpecial) unitListCapacity = 12;
    else unitListCapacity = 8;

    unitList = new UnitList(unitListCapacity);
    for (int i = 0; i < size; i++) {
        unitList->insert(unitArray[i]);
    }
}

int Army::myCeil(double value) {
    double diff = abs(value - round(value));
    return (diff < 1e-9) ? round(value) : ceil(value);
}
...
UnitList::UnitList(int capacity) : infantryCount(0), vehicleCount(0), head(nullptr) {
...
UnitList* UnitList::shallowReverseCopy() const {
    UnitList* newUnitList = new UnitList(capacity);
    Node* cursor = head;
    while (cursor != nullptr) {
...
void UnitList::reduceWeight(double percentage) {
    Node* cursor = head;
    while (cursor != nullptr) {
        cursor->unit->setWeight(myCeil(cursor->unit->getWeight() * (1 - percentage)));
        cursor = cursor->next;
    }
}
...
void UnitList::reduceQuantity(double percentage) {
    Node* cursor = head;
    while (cursor != nullptr) {
        cursor->unit->setQuantity(myCeil(cursor->unit->getQuantity() * (1 - percentage)));
        cursor = cursor->next;
    }
}
...
int UnitList::myCeil(double value) {
    double diff = abs(value - round(value));
    return (diff < 1e-9) ? round(value) : ceil(value);
}
    ...
    if (dynamic_cast<LiberationArmy*>(army)) {
        ...
        while (cursor != nullptr) {
            if (distance(this->pos, cursor->unit->getCurrentPosition()) <= 2 && dynamic_cast<Infantry*>(cursor->unit)) {
                army->setEXP(army->getEXP() + myCeil(cursor->unit->getStaticAttackScore() * 30.0/100.0));
            }
            else if (distance(this->pos, cursor->unit->getCurrentPosition()) <= 2 && dynamic_cast<Vehicle*>(cursor->unit)) {
                army->setLF(army->getLF() - myCeil(cursor->unit->getStaticAttackScore() * 10.0/100.0));
            }
            cursor = cursor->next;
        }
    }
        ...
        while (cursor != nullptr) {
            if (distance(this->pos, cursor->unit->getCurrentPosition()) <= 4 && dynamic_cast<Infantry*>(cursor->unit)) {
                army->setEXP(army->getEXP() + myCeil(cursor->unit->getStaticAttackScore() * 20.0/100.0));
            }
            else if (distance(this->pos, cursor->unit->getCurrentPosition()) <= 4 && dynamic_cast<Vehicle*>(cursor->unit)) {
                army->setLF(army->getLF() - myCeil(cursor->unit->getStaticAttackScore() * 5.0/100.0));
            }
            cursor = cursor->next;
        }
    }
...
void River::getEffect(Army *army) {
    ...
    while (cursor != nullptr) {
        if (distance(this->pos, cursor->unit->getCurrentPosition()) <= 2 && dynamic_cast<Infantry*>(cursor->unit)) {
            cursor->unit->setAttackScore(cursor->unit->getStaticAttackScore() - myCeil(cursor->unit->getStaticAttackScore() * 0.1));
        }
        cursor = cursor->next;
    }
}
        ...
        while (cursor != nullptr) {
            if (Infantry* infantry = dynamic_cast<Infantry*>(cursor->unit)) {
                ...
                if ((unitDistance <= 5 && unitDistance != 0.0) && (infantry->getInfantryType() == SPECIALFORCES || infantry->getInfantryType() == REGULARINFANTRY)) {
                    infantry->setAttackScore(infantry->getStaticAttackScore() + myCeil(infantry->getStaticAttackScore() * 2 / unitDistance));
                }
            }   
                ...
                if (unitDistance <= 2 && vehicle->getVehicleType() == ARTILLERY) {
                    vehicle->setAttackScore(vehicle->getStaticAttackScore() - myCeil(vehicle->getStaticAttackScore() * 0.5));
                }
            }
            cursor = cursor->next;
        }
        ...
        while (cursor != nullptr) {
            if (Infantry* infantry = dynamic_cast<Infantry*>(cursor->unit)) {
                ...
                if ((infantry->getInfantryType() == REGULARINFANTRY) && (unitDistance <= 3 && unitDistance != 0.0)) {
                    infantry->setAttackScore(infantry->getStaticAttackScore() + myCeil(infantry->getStaticAttackScore() * 3.0 / (2.0 * unitDistance)));    
                }
            }
            cursor = cursor->next;
        }
    ...
    if (dynamic_cast<LiberationArmy*>(army)) {
        ...
        while (cursor != nullptr) {
            if (distance(this->pos, cursor->unit->getCurrentPosition()) <= 2) {
                cursor->unit->setAttackScore(cursor->unit->getStaticAttackScore() - myCeil(cursor->unit->getStaticAttackScore() * 0.2));
            }
            cursor = cursor->next;
        }
    }
